...
class ContentAnalyzer {
    analyze(input) {
        const tokens = marked_1.marked.lexer(input.content, { gfm: true });
    ...
    headingSignificance(depth, text) {
    ...
    rankSentences(sentences, technicalDetails) {
    ...
    extractKeyPoints(input) {
        ...
        const listPoints = this.extractListItems(input.tokens).map((item) => ({
            text: item,
    ...
    extractTechnicalDetails(content, codeExamples) {
    ...
    collectMatchedTerms(content, lowerContent, candidates) {
    ...
    preserveReadableCasing(content, candidate) {
    ...
    extractCommands(content, codeExamples) {
    ...
    countTechnicalTerms(text) {
        const lowerText = text.toLowerCase();
        return [...LANGUAGE_TERMS, ...FRAMEWORK_TERMS, ...CONCEPT_TERMS].reduce((count, term) => count + (lowerText.includes(term.toLowerCase()) ? 1 : 0), 0);
    }
    inferComplexity(input) {
        let score = 0;
    ...
    inferAudience(content, codeBlockCount) {
    ...
    selectIcon(title, filePath, codeBlockCount) {
