...
class SummaryGenerator {
    generateDocument(results, options) {
        const lines = [];
        const orderedResults = [...results].sort((left, right) => left.metadata.filePath.localeCompare(right.metadata.filePath));
        ...
        lines.push(`- Intermediate: ${orderedResults.filter((result) => result.metadata.complexity === 'intermediate').length}`);
        ...
        for (const result of orderedResults) {
            ...
            lines.push(`- ${result.metadata.icon} [${result.metadata.title}](#${anchor}) — ${this.complexityBadge(result.metadata.complexity)}`);
        }
    ...
    renderGroupedSections(lines, results) {
    ...
    renderResult(lines, result) {
        ...
        lines.push('');
        lines.push(`- File: ${path_1.default.basename(result.metadata.filePath)}`);
    ...
    renderTechnicalDetails(lines, details) {
        const sections = [
    ...
    significanceMarker(significance) {
